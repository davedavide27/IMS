...
class Login extends DBConnection {
    ...
    public function login(){
        ...
        if ($qry->num_rows > 0) {
            ...
            // Set login type based on user type
            switch ($res['type']) {
                case 1:
                    $this->settings->set_userdata('login_type', 1); // Administrator
                    break;
                case 2:
                    $this->settings->set_userdata('login_type', 2); // Staff
                    break;
                case 3:
                    $this->settings->set_userdata('login_type', 3); // Manager
                    break;
                default:
                    return json_encode(array('status' => 'unauthorized', 'msg' => 'Unauthorized user type.'));
            }

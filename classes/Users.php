...
class Users extends DBConnection
{
	...
	public function save_users()
	{
		...
		// Build data string for query
		foreach ($_POST as $k => $v) {
			if (in_array($k, ['firstname', 'middlename', 'lastname', 'username', 'type', 'status'])) {
				if (!empty($data)) $data .= " , ";
				$data .= " {$k} = '{$v}' ";
			}
		}
		...
		if (!empty($password)) {
			$password = md5($password); // Use MD5 for hashing
			if (!empty($data)) $data .= " , ";
